...
Item {
  ...
  readonly property string selectedIconSource: {
    var e = root.currentEntry()
    return (e && e.cls) ? root.iconSourceFor(e.cls) : ""
  }
  ...
  function resolveClass(cls) {
    ...
    for (var o = 0; o < root.iconOverrides.length; o++) {
      ...
      if (String(ov.cls).toLowerCase() === needle) {
        hit = { source: ov.source, name: ov.name || root.friendlyClass(clsName) }
        break
      }
    }
    if (hit.source === "") {
      ...
      outer: for (var i = 0; i < values.length; i++) {
        ...
        if (sc === needle || name === needle || id === needle
          ...
          hit = { source: root.resolveEntryIcon(entry.icon), name: entry.name || clsName }
          break outer
        }
      }
    }
        ...
        Item {
          ...
          Repeater {
            ...
            Item {
              ...
              readonly property string iconSource: (card.modelData && card.modelData.cls)
                ? root.iconSourceFor(card.modelData.cls) : ""
